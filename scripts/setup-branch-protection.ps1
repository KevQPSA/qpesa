...
if (-not $env:GITHUB_TOKEN) {
    ...
    Write-Host "Please set your GitHub Personal Access Token first" -ForegroundColor Yellow
    exit 1
}
...
try {
    $mainResponse = Invoke-RestMethod -Uri "$githubApi/repos/$repoOwner/$repoName/branches/main/protection" -Method Put -Headers $headers -Body $mainProtection -ContentType "application/json"
...
try {
    ...
    try {
        ...
        $developCreateResponse = Invoke-RestMethod -Uri "$githubApi/repos/$repoOwner/$repoName/git/refs" -Method Post -Headers $headers -Body $createDevelop -ContentType "application/json"
        Write-Host "Develop branch created successfully" -ForegroundColor Green
    } catch {
        Write-Host "Could not create develop branch: $($_.Exception.Message)" -ForegroundColor Yellow
    }
}
...
try {
    $developResponse = Invoke-RestMethod -Uri "$githubApi/repos/$repoOwner/$repoName/branches/develop/protection" -Method Put -Headers $headers -Body $developProtection -ContentType "application/json"
